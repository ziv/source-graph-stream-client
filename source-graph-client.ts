...
/**
 * Re-export SSEMessage type from sse-stream-transform
 * Used as yield value in raw() method.
 *
 * @see https://www.npmjs.com/package/sse-stream-transform
 */
 ...
 * @see https://sourcegraph.com/docs/api/stream_api#event-stream-format
 */
...
export class SourceGraphClient {
  ...
  async *search(
    ...
    for await (const msg of read.pipeThrough(new SseStreamTransform())) {
      ...
      const results = safeParse<SearchResult[]>(msg.data);
   ...
   * @see https://sourcegraph.com/docs/api/stream_api#request
   *
   * @param query
   * @param options
   * @protected
